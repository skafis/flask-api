...
class ShoppingList(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.shopinglist = {'title': 'Grocery'}
    ...
    def test_list_creation(self):
        ...
        res = self.client().post('/shopinglists/', data=self.shopinglist)
    ...
    def test_get_all_list(self):
        ...
        res = self.client().post('/shopinglists/', data=self.shopinglist)
        self.assertEqual(res.status_code, 201)
        res = self.client().get('/shopinglists/')
        self.assertEqual(res.status_code, 200)
        self.assertIn('Grocery', str(res.data))

    def test_api_get_by_id(self):
        """Test API can get list by using it's id."""
        rv = self.client().post('/shopinglists/', data=self.shopinglist)
    ...
    def test_list_is_editable(self):
        ...
        rv = self.client().post(
            '/shoppinglists/',
            data={'title': 'meaty things'})
        self.assertEqual(rv.status_code, 201)
        rv = self.client().put(
            '/shoppinglists/1',
            data={
                "title": "sweet meaty objects"
            })
